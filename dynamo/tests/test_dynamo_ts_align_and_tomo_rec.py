...
from dynamo.protocols import DynamoTsAlignAndTomoRec
...
class TestDynamoAliRecBase(TestBaseCentralizedLayer):
    ds = None
    importedTs = None
    ...
    def setUpClass(cls):
        ...
        cls.expectedOriginShifts = list(np.array(cls.expectedTomoDims) / -2 * cls.unbinnedSRate * cls.binFactor4)
...
class TestDynamoAliRec(TestDynamoAliRecBase):

    def test_align_01(self):
        # Binned dims X and Y are always odd in Dynamo
        ...
        prot = self.newProtocol(DynamoTsAlignAndTomoRec,
                                inputTs=self.inTsSet,
                                beadDiamNm=10,
                                recTomo=False,
